...
玩家扮演不同建筑公司，在 6×6 的城乡居民点地图中修建道路和桥梁，使 6 位商人能够从城市到乡村或从乡村到城市完成交易。商人会沿已建道路网络中的最短路径移动，并向经过路段、桥梁的所有者支付过路费。第 6 位商人完成交易后游戏结束，累计过路费最高者获胜；若过路费相同，则已建设道路数量更多者获胜。
...
| 卡牌 | 概率 | 概率权重 | 效果 |
|---|---:|---:|---|
| 随机修路卡 | 2/5 | 2 | 从全部60 条边中无放回随机选择两条不同的边，逐条判断，所有合法边均修建道路 |
| 桥梁通路卡 | 1/5 | 1 | 将自己还未修路的一座桥梁修好路 |
| 拆路修桥卡 | 1/5 | 1 | 将自己的一条路拆掉作为原材料，在地图任意位置修建一座桥梁 |
| 资金补贴卡 | 1/5 | 1 | 直接获得 3$ 过路费 |
...
效果：系统从全部60 条相邻边中无放回随机选择两条不同的边，逐条判断当前玩家是否满足修路条件；所有满足条件的边均立即修建道路，不满足条件的边无效果。

程序流程建议：

1. 获取地图上的全部60 条相邻边；
2. 使用无放回抽样随机选择 2 条不同的边，不先筛选合法边；
3. 依次判断每条边是否满足当前玩家的修路条件；
4. 对每条合法边立即修建道路，对每条非法边记录无效果；
5. 两条边都不合法时，地图不发生变化，但卡牌仍视为已结算。

> 这里的“随机”作用于全部边，而不是仅作用于当前可修路边。两条边分别判定，因此一次卡牌可能修建 0、1 或 2 条道路。
...
第 4、5 位大商人即使偶然抽到 `(6,6) → (1,1)`，也仍属于随机路线，不改变第 6 位的固定路线规则。
...
随机修路卡抽取两条边后逐条判定，不会因为其中一条边非法而跳过另一条边：

- 若某条边满足当前玩家的修路条件，则立即修建道路；
- 若某条边是跨河边但没有当前玩家自己的桥梁，或该边已有道路，则该条边无效果；
- 两条边分别判定，合法边全部建造，因此一次卡牌可能建造 0、1 或 2 条道路。
...
1. 随机修路卡从全部60 条边中抽取两条不同的边；
2. 随机修路卡逐条判定，两条中合法的边全部修建，非法边不改变地图；
3. 随机修路卡两条边均非法时无地图变化；
4. 桥梁通路卡只能作用于自己的有桥无路边；
5. 拆路修桥卡在无自有道路时无效果；
6. 拆路修桥卡不能在非跨河边修桥；
7. 资金补贴卡增加 3$。
...
| 行动 3 跨河已有自己桥时 | 可修路 |
| 行动 3 跨河已有他人桥时 | 不可操作 |
| 随机修路卡非法时 | 两条边分别判定，合法边全部修建；仅两条边均非法时没有地图变化 |
