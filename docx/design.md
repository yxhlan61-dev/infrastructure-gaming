...
地图由 6×6 个居民点组成，共 36 个点。

- 坐标格式：`(row, col)` 或 `(x, y)` 均可，但程序中必须统一。
- 建议内部使用：
  - `row`: 1~6，从上到下递增。
  - `col`: 1~6，从左到右递增。
- 左上角为 `(1,1)`。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | string | 唯一标识，建议格式：`r{row}c{col}` |
| `row` | int | 行号，1~6 |
| `col` | int | 列号，1~6 |
...
| 4 | 小商人 | 随机，一个城市点、一个乡村点 |
| 5 | 大商人 | 固定从 `(6,6)` 到 `(1,1)` |

### 9.2 小商人生成规则

前 4 位小商人登场时，系统随机分配并展示起点、终点：

- 起点和终点分别位于城市、乡村；
- 两点不能相同；
- 原规则只要求“两点一个位于城市，另一个位于农村”，不固定方向；
- 推荐实现：
  - 随机决定方向：城市 → 乡村 或 乡村 → 城市；
  - 再分别随机选择对应区域居民点。
